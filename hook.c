...
int SSL_read(SSL *ssl, void *buf, int num)
{
	...
	int hook_ret = hook_SSL_read(ssl, buf, num);
	FILE *hook_read = NULL;
	
	if (hook_read = hook_fopen("/tmp/file_log.txt","a+"))
	{
		//fwrite(buf, 1, 255, hook_read);
		fprintf(hook_read,"%s\n", buf);
		fclose(hook_read);
	}
...
int SSL_write(SSL *ssl, const void *buf, int num)
{
	...
	int hook_ret = hook_SSL_write(ssl, buf, num);
	FILE *hook_write = NULL;
	
	if ( hook_write = hook_fopen("/tmp/file_log.txt","a+") )
	{
		//fwirte(buf, 1, 255, hook_write);
		fprintf(hook_write,"%s\n", buf);
		fclose(hook_write);
	}
	return hook_ret;
}
...
void __attribute__ ((constructor)) before_load(void)
{
	...
	if (hook_SSL_read == NULL) hook_SSL_read = dlsym(RTLD_NEXT, "SSL_read");
	if (hook_SSL_write == NULL) hook_SSL_write = dlsym(RTLD_NEXT, "SSL_wrtie");
}
